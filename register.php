...
$conn = mysqli_connect($_SERVER , $usernam , $password ,$dbname) or die('connection failed'); 
...
if(isset($_POST['submit_re'])){
    ...
    // The mysqli_real_escape_string () function is an inbuilt function
    //  in PHP which is used to escape all
    //  special characters for use in an SQL query
   // $name = mysqli_real_escape_string($conn, $_POST['name']);   
   // $email = mysqli_real_escape_string($conn, $_POST['email']);
   // $pass = mysqli_real_escape_string($conn, md5($_POST['password']));
   // $cpass = mysqli_real_escape_string($conn, md5($_POST['cpassword']));

   $select = mysqli_query($conn, "SELECT * FROM `user_form` WHERE mail = '$email' AND password = '$pass'") or die('query failed');

   if(mysqli_num_rows($select) > 0){
      $message[] = 'user already exist!';
   }else{
      mysqli_query($conn, "INSERT INTO `user_form`(name, mail, password) VALUES('$name', '$email', '$pass')") or die('query failed');
...
<div class="form-container">
   ...
   <form action="register.php" method="post">
      ...
      <input type="submit" name="submit_re" class="btn" value="register now">
